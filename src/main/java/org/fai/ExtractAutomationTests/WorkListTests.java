...
public class WorkListTests extends BaseTest{
	...
	@Test(priority=28,groups={"smoke"},dependsOnGroups = {"prerequisites"})
	...
	@Test(priority=29,groups={"smoke"},dependsOnGroups = {"prerequisites"})
	...
	public void verifyAutoIngestionFileStatus(Map<String, String> input) throws AWTException {
		new NavigationBar()
		.gotoWorkList();	
		new WorkList().clearInputText().searchFiles(input.get("searchText")).waitForFilestatusChange(input.get("searchText"), input.get("exp status"));
